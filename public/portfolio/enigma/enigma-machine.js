...
class EnigmaMachine extends GestureEventListeners(PolymerElement) {
            ...
            <div class="int-page int-vertical-container">
                ...
                <iron-pages selected="[[data.mode]]" attr-for-selected="mode">
                    <section class="int-vertical-container" mode="encrypt">
                        ...
                        <textarea
                            id="decrypted-message-input"
                            class="message-area"
                            value="{{messageToEncrypt::input}}"
                        ></textarea>

                        <h2>Enter a question:</h2>
                        <iron-input
                            bind-value="{{encryptingQuestion}}"
                            on-keydown="encryptOnEnter"
                        >
                        ...
                        <iron-input
                            bind-value="{{encryptingAnswer}}"
                            on-keydown="encryptOnEnter"
                        >
                            <input value="{{encryptingAnswer::input}}" />
                        </iron-input>
                        <button
                            class="int-button int-button--block"
                            on-tap="encrypt"
                        >
                            Encrypt
                        </button>
                        ...
                        <a
                            class="int-button"
                            href="data:text/plain;charset=utf-8,[[encodedEncryptedMessage]]"
                            download="encodedMessage.txt"
                            disabled$="[[!encryptedMessage]]"
                            >Download</a
                        >
                    </section>
                    <section class="int-vertical-container" mode="decrypt">
                        ...
                        <p>
                            <sup
                                >*or type it if you love hunting for unicode
                                characters</sup
                            >
                        </p>
                        <label for="encrypted-message-upload" class="int-button"
                            >Upload</label
                        >
                        <input
                            id="encrypted-message-upload"
                            class="int-button"
                            type="file"
                            accept=".txt"
                            on-change="upload"
                        ...
                        <textarea
                            ...
                            class="message-area"
                            value="{{questionAndMessageToDecrypt::input}}"
                        ></textarea>
                        ...
                        <iron-input
                            bind-value="{{decryptingAnswer}}"
                            on-keydown="decryptOnEnter"
                        >
    ...
    connectedCallback() {
        ...
        this.set(
            "data",
            this.data.mode === "" ? { mode: "encrypt" } : this.data
        );
    }
    ...
    encrypt() {
        ...
        const encryptedMessage = this.messageToEncrypt
            .split("")
            .map((letter, index) => {
                return String.fromCharCode(
                    letter.charCodeAt() + key[index % key.length].charCodeAt()
                );
            })
            .join("");
        this.set(
            "encryptedMessage",
            `${this.encryptingQuestion}\n\n${encryptedMessage}`
        );
    }
    ...
    decrypt() {
        ...
        const decryptedMessage = this.messageToDecrypt
            .split("")
            .map((letter, index) => {
                return String.fromCharCode(
                    letter.charCodeAt() - key[index % key.length].charCodeAt()
    ...
    copy() {
        const encryptedOutput = this.shadowRoot.querySelector(
    ...
    upload(e) {
        ...
        reader.onload = (() => {
            return (e) => {
                this.set("questionAndMessageToDecrypt", e.target.result);
            };
        })();
